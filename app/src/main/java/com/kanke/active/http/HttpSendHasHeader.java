...
public class HttpSendHasHeader {

    private HttpSendHasHeader() {
    }
    ...
    static HttpRes sendGetMsg(HttpClient httpClient, AbsRequst msg, int sendCount, int maxSendCount) {
        ...
        try {
            ...
            if (Configurations.DEBUG)
                Logger.d(HttpSend.class, "[HTTP REST消息发送工具类]：GET消息：第", sendCount, "次发送，Request URI:", uri, "，Token:",
            ...
            Logger.d(HttpSend.class, "[HTTP REST消息发送工具类]：GET消息响应，statusCode：", httpRes.getStatusCode(), "，content:",
     ...
     * @param httpRequest
     */
    private static void setBizHeader(AbsRequst msg, HttpRequestBase httpRequest) {
        httpRequest.setHeader("Content-Type", "application/json;charset=UTF-8");

        if (msg.mAccessToken != null) {
            httpRequest.setHeader("token", msg.mAccessToken);
        }
    }
